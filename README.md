...
还有一个非常不好的开发习惯，就是很多同学喜欢把项目中的一些基础共用的数据或者配置信息直接在代码里面写死，这个是非常不理智的行为，当出现一些服务环境的之间的切换，数据变更的时候，你需要去频繁修改代码，所以我更建议将一些固定的数据、统一收归到配置文件中去。 实现配置的方式有很多，比如在项目中新建一个配置文件，或者通过mysql、etcd、consul、zk等存储。
